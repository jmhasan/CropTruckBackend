...
from masterdata.models import CustomerProfile, CompanyProfile
...
class CertificateService:
    ...
    @staticmethod
    def calculate_amounts(token, validated_data):
        """Calculate all financial amounts based on token and input data"""
        number_of_sacks = token.xsack  # Get from token
        rent_per_sack = validated_data.get('rent_per_sack', 0) or 0
        advance_rent = validated_data.get('advance_rent', 0) or 0
        price_of_empty_sacks = validated_data.get('price_of_empty_sacks', 0) or 0
        number_of_empty_sacks = validated_data.get('number_of_empty_sacks', 0) or 0
        transportation = validated_data.get('transportation', 0) or 0
        given_loan = validated_data.get('given_loan', 0) or 0

        # Calculate amounts
        total_rent = number_of_sacks * rent_per_sack
        remaining_rent = total_rent - advance_rent
        total_empty_sacks_amount = number_of_empty_sacks * price_of_empty_sacks
        total_amount_taka = remaining_rent + total_empty_sacks_amount + transportation + given_loan

        return {
            'number_of_sacks': number_of_sacks,
            'total_rent': total_rent,
            'remaining_rent': remaining_rent,
            'total_empty_sacks_amount': total_empty_sacks_amount,
            'total_amount_taka': total_amount_taka
        }
    ...
    def create_certificate(validated_data, user):
        ...
        # Step 1: Validate token availability and get token
        token = CertificateService.validate_token(business_id, token_no)

        # Step 2: Calculate all amounts based on token and input data
        calculated_amounts = CertificateService.calculate_amounts(token, validated_data)
        print(calculated_amounts['number_of_sacks'])
        ...
        # Step 4: Get or create customer profile
        customer, is_new_customer = CertificateService.get_or_create_customer(business, xmobile, customer_data)

        # Step 5: Create certificate with customer information and calculated amounts
        certificate_data = {
            'business_id': business,
            'token_no': token_no,
            'certificate_no': validated_data.get('certificate_no'),
            ...
            'village': customer.village,
            'post_office': customer.post_office,
            'number_of_sacks': calculated_amounts['number_of_sacks'],
            'potato_type': validated_data.get('potato_type'),
            'rent_per_sack': validated_data.get('rent_per_sack'),
            'total_rent': calculated_amounts['total_rent'],
            'advance_rent': validated_data.get('advance_rent'),
            'remaining_rent': calculated_amounts['remaining_rent'],
            'number_of_empty_sacks': validated_data.get('number_of_empty_sacks'),
            'price_of_empty_sacks': validated_data.get('price_of_empty_sacks'),
            'transportation': validated_data.get('transportation'),
            'given_loan': validated_data.get('given_loan'),
            'total_amount_taka': calculated_amounts['total_amount_taka'],
            'created_by': user,
            'created_at': timezone.now()
        }
